...
| Server-default RPC compatibility bridge | Fork core | Normal CLI RPC must enter the daemon-owned session boundary for durable state and operation ownership while preserving prompt follow-up semantics; the legacy direct RPC entry point remains available for v1 compatibility. Compatibility is covered by the server-default RPC production fixtures. |
