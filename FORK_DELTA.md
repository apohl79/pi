...
| Remote TUI views and interactive adapter | Fork-dependent extension | Existing TUI components can render server snapshots, but remote leases, controls, live name/phase/usage/goal/compaction/instruction projections, and server-owned active agent paths require a v2 client adapter. |
| Harness structural lifecycle events | Fork core | Compaction and tree navigation are server-owned durable operations; the stock harness event surface does not expose their accepted run ID, reason, terminal outcome, or sanitized failure after persistence. Compatibility is covered by the AgentHarness compaction, navigation, and passive run-event conformance tests. |
