...
| --- | --- | --- |
| Protocol v2 schemas, CBOR framing, operation/event cursors | Fork core | Stock Pi has no durable daemon command model or event-cursor contract; authoritative session-name changes are emitted as persisted `session_name_updated` events after runtime state transitions. Compatibility is covered by `packages/server/test/v2-conformance.test.ts`. |
