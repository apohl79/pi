...
describe("coding-agent daemon plugin commands", () => {
	test("expands a plugin command as a prompt template", async () => {
		expect(await runPluginCommandScenario()).toBe(true);
	});
});

async function runPluginCommandScenario(): Promise<boolean> {
	...
	const faux = fauxProvider({
		provider: "coding-agent-daemon-plugin-commands-faux",
		models: [{ id: "plugin-commands-model", reasoning: false, contextWindow: 32_000, maxTokens: 1_000 }],
	});
	...
	faux.setResponses([
		(context: Context) => {
			...
			return fauxAssistantMessage("command response");
		},
	]);
	...
	try {
		...
		await client.request({
			...
			payload: {
				...
				version: "1.0.0",
				root: pluginRoot,
				manifest: { name: "review-plugin", version: "1.0.0", commands: ["commands/review.md"] },
			},
		});
		...
		await client.request({
			command: "turn/start",
			sessionId,
			payload: { text: "/review-plugin@local:review changed files" },
		});
		await waitForIdle(client, sessionId);
		return userMessages.some((message) => message.includes("Review changed files"));
	} finally {
		client.dispose();
		await runtime.close();
	}
